...
bot.message do |event|
  puts "[#{event.server&.name || 'DM'}] #{event.author.distinct} : #{event.message.content}"
  puts "[#{event.author.bot_account?}"
  next if event.author.bot_account?
  puts "[#{event.message.content.start_with?(PREFIX)}"
  next unless event.message.content.start_with?(PREFIX)
  
  command = event.message.content[PREFIX.length..].strip
  puts "[#{command}"
  next if command.empty?

  event.channel.start_typing
  result = download_first_bing_pokemon_image(command)
  ...
  event.channel.send_file(StringIO.new(image_bytes), filename: filename)
end
